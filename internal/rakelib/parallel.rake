...
# TMPDIR per run, which is what a CI job actually looks like. One pair per run
# rather than per worker, because a CI job is one machine with several workers
# on it: a HOME per worker would be more isolated than CI and would hide the
# very races it is meant to expose.
#
...
task(:test_soak) do
  ...
  puts("Each run had a throwaway HOME and TMPDIR.") if isolate
...
#
# This is a much better detector than test_soak for anything cross process, and
